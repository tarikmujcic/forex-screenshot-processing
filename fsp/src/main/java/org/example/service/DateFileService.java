...
//    }
    ...
    public static String determineFileName() {
        ...
        String startDateFormatted = startDate.format(DateTimeFormatter.ofPattern("MM-dd"));
        System.out.println("startDateFormatted: " + startDateFormatted);

        // Inline end-date logic (today)
        LocalDate endDate = LocalDate.now();

        String endDateFormatted = endDate.format(
                DateTimeFormatter.ofPattern("MM-dd")
        );

        String yearShort = String.valueOf(startDate.getYear()).substring(2);

        String outputFileName =
                startDateFormatted + "To" + endDateFormatted + "-" + yearShort + ".png";
